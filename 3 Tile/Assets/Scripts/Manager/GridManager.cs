...
public class GridManager : MonoBehaviour
{
    ...
    private void CreateBrick()
    {
       /* Level currentLevel = PrefabStorage.instance.DataLevel.GetLevel(PlayerDataManager.GetCurrentLevel());
        ...
        for (int i = 0 l i < currentLevel.listGrp.Count; i++)
        {
            PlaceGroup(currentLevel.listGrp[i].place.x, currentLevel.listGrp[i].place.y, currentLevel.listGrp[i].groupType);
        }

        for (int i = 0 l i < currentLevel.listGridPlace.Count; i++)
        {
            PlaceBox(currentLevel.listGridPlace[i].place.x, currentLevel.listGridPlace[i].place.y, currentLevel.listGridPlace[i].z);
        }*/
    }
    ...
    private void CreateDataConceptAndAmoutTypeInLevel()
    {
        ...
        var firstThreeElements = enumValues.Take(PrefabStorage.instance.DataLevel.GetAmoutTypeInLevel(PlayerDataManager.GetCurrentLevel()));
    ...
    {
        /*for (int i = 0; i < listInCubeLevel.Count; i++)
        {
            if (listInCubeLevel[i].CheckGridOverLap())
            {
                listInCubeLevel[i].TurnWhite();
            }
            else
            {
                listInCubeLevel[i].TurnBlack();
            }
        }*/
    }
